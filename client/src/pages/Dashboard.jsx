...
export default function Dashboard() {
  ...
  const unpaidPayments = payments.filter(p => p.editor_paid !== 'paid' || p.client_paid !== 'cobrado');
  const totalEditorPending = unpaidPayments.filter(p => p.editor_paid !== 'paid').reduce((s, p) => s + getEditorTotal(p), 0);
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px 6px' }}>
        ...
        {unpaidPayments.slice(0, PAYMENTS_PREVIEW_LIMIT).map((p, i) => {
          const missingEditor = p.editor_paid !== 'paid';
